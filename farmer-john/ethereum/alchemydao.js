...
const { init_ethers, loadSynthetixPool, getToken, getParameterCaseInsensitive, lookUpTokenPrices, getPoolPrices, printSynthetixPool, formatMoney } = require('./ethers_helper');
...
module.exports = async function main() {
  ...
  console.log(`Total staked: $${formatMoney(p0.staked_tvl + p1.staked_tvl)}`);
  if (p0.totalUserStaked > 0) {
    console.log(`You are staking a total of $${formatMoney(p0.totalUserStaked)} at an APR of ${(p0.totalAPR * 100).toFixed(2)}%\n`);
  }

  p0.name = 'Synthetix';
  p1.name = 'Alchemy';
  try {
    p0.price = prices[tokens[Object.keys(tokens)[0]].address];
  } finally { }
  try {
    p1.price = prices[tokens[Object.keys(tokens)[1]].address];
  } finally { }
