...
async function printLiquidityMiningInfo(App, info, rewardTokenTicker, rewardTokenPrice, rewardsProxy, wrapper) {
  ...
  console.log(`Stake ${info.userAvailableToStake.toFixed(18)} ${info.marketSymbol}`, approveAndStake)
  console.log(`Withdraw ${info.userStaked.toFixed(18)} ${info.marketSymbol}`, withdraw)
  console.log(
...
async function printSingleSidedInfo(App, info, rewardTokenTicker, rewardTokenAddress, rewardTokenPrice, wrapper) {
  ...
  console.log(`Stake ${formatMoney(info.userAvailableToStake / 1e18)} ${rewardTokenTicker}`, approveAndStake)
  console.log(`Unstake ${formatMoney(info.userShare / 1e18)} ${rewardTokenTicker}`, unstake)
  console.log(`Revoke (set approval to 0)`, revoke)
  _print('')
}
