...
const loadVSDDAO = async (App, DAO, DOLLAR) => {
    ...
    console.log(`Redeem ${unstaked.toFixed(decimals)} ${dollar}`, approveAndRedeem);
    _print('');
...
async function loadValueSetLP(App, DAO, LP, fluidEpochs, epoch, rewardTicker, uniPrices, dollarPrice, unstakedDollar) {
    ...
    console.log(`Deposit ${unstaked.toFixed(8)} ${LP.ticker}`, approveAndDeposit)
    console.log(`Withdraw ${staged.toFixed(8)} ${LP.ticker}`, withdraw);
    console.log(`Bond ${staged.toFixed(8)} ${LP.ticker}`, bond);
    console.log(`Unbond ${bonded.toFixed(8)} ${LP.ticker}`, unbond);
    console.log(`Claim ${claimable.toFixed(decimals)} ${rewardTicker}`, claim);
    console.log(`Provide ${(vsdToDep).toFixed(decimals)} ${rewardTicker} + ${baseToDep} ${LP.baseCoinTicker}`, provide);
    _print('');
}
