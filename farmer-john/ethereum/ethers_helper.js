...
const { GELATO_ABI, UNI_ABI, BALANCER_POOL_ABI, JAR_ABI, CTOKEN_ABI, HARVEST_VAULT_ABI,
  SADDLE_LP_TOKEN_ABI, CURVE_ABI, STABLESWAP_ABI, ERC20_ABI, DSTOKEN_ABI } = require('../abis');

const networkNameFromId = function (id) {
  for(let network of Object.values(networks.NETWORKS)) {
...
const pageNetwork = function(network = '') {
  if (network.toLowerCase() === 'bsc') {
    return networks.NETWORKS.BINANCE_SMART_CHAIN
  }
  if (network.toLowerCase() === 'heco') {
    return networks.NETWORKS.HECO
  }
  if (network.toLowerCase() === 'polygon') {
    return networks.NETWORKS.POLYGON
  }
  if (network.toLowerCase() === 'okex') {
    return networks.NETWORKS.OKEX
  }
  if (network.toLowerCase() === 'kcc') {
    return networks.NETWORKS.KCC
  }
  if (network.toLowerCase() === 'xdai') {
    return networks.NETWORKS.XDAI
  }
  if (network.toLowerCase() === 'fantom') {
    return networks.NETWORKS.FANTOM
  }
  if (network.toLowerCase() === 'harmony') {
    return networks.NETWORKS.HARMONY_S0
  }
  if (network.toLowerCase() === 'avax') {
    return networks.NETWORKS.AVALANCHE
  }
  if (network.toLowerCase() === 'fuse') {
    return networks.NETWORKS.FUSE
  }
  if (network.toLowerCase() === 'thundercore') {
    return networks.NETWORKS.THUNDERCORE
  }

  return networks.NETWORKS.ETHEREUM
}
...
const init_wallet = async function (callback) {
  ...
  if (walletProvider) {
    ...
    if (connectedNetwork.chainId === targetNetworkId) {
      ...
      if (networks.ethereum && targetNetwork.chainId !== '0x1') {
        _print('')
...
async function init_ethers(nodeUrl = atob(networks.ETHEREUM_NODE_URL)) {
  const App = {
    YOUR_ADDRESS: '0x0000000000000000000000000000000000000000'
  }

  try {
    App.provider = new ethers.providers.JsonRpcProvider(nodeUrl);
    App.ethcallProvider = new ethcall.Provider();
    await App.ethcallProvider.init(App.provider);
  } catch (e) {
    console.log("App not initialized:", e, JSON.stringify(e, Object.getOwnPropertyNames(e)));
  }
...
const switchNetwork = async function(network) {
  await networks.ethereum.request({method: 'wallet_addEthereumChain', params: [network]}).catch()
  networks.location.reload()
}

const changeWallet = async function() {
  let cached = networks.web3Modal.cachedProvider
  networks.web3Modal.clearCachedProvider()
  await connectWallet(()=> networks.location.reload() )
  if (!networks.web3Modal.cachedProvider) {
    networks.web3Modal.setCachedProvider(cached)
  }
}

const connectWallet = async function(callback) {
  try {
    walletProvider = await networks.web3Modal.connect()
...
const getUrlParameter = function(sParam) {
  let sPageURL = networks.location.search.substring(1),
...
const _print_link = function(message, onclickFunction, uuid = ID(), add_carriage = true) {
  console.log( '<a href="#" id=' + uuid + '>' + message + '</a>')
  if (add_carriage) {
    // console.log( '')
  }
}
...
const _print_href = function(message, href) {
  ...
  console.log( `<a href="${href}" target="_blank" id="${uuid}">${message}</a>`)
}
...
const lookUpTokenPrices = async function(id_array) {
  const prices = {}
  try {
    const axiosClient = rateLimit(axios.create(), { maxRPS: 100 / 60 });
    for (const id_chunk of chunk(id_array, 50)) {
      let ids = id_chunk.join('%2C')
      let res = await axiosClient.get('https://api.coingecko.com/api/v3/simple/token_price/ethereum?contract_addresses=' + ids + '&vs_currencies=usd')
      for (const [key, v] of Object.entries(res.data)) {
        if (v.usd) prices[key] = v;
      }
    }
  } catch (err) {
    console.log('lookUpTokenPrices:', err);
  }
  return prices
}
...
const goToNext = function() {
  const href = networks.location.href.split('/');
  const chain = href[3];
  const pools = JSON.parse(localStorage.getItem(`${chain}Urls`));
  if (pools && pools.length > 0 && href.length == 5 && href[href.length - 1] != '') {
    const pool = href[href.length - 1];
    const nextPool = pools.indexOf(pool) + 1;
    if (nextPool >= pools.length) {
      console.log('Stopping... when to continue?');
    } else {
      console.log("Go to:", `/${chain}/${pools[nextPool]}`)
      networks.location.href = `/${chain}/${pools[nextPool]}`;
    }
  } else {
    console.log("Go to:", `/${chain}/${pools[0]}`)
    networks.location.href = `/${chain}/${pools[0]}`;
  }
}
...
async function getUniPool(app, pool, poolAddress, stakingAddress) {
  ...
  try {
    ...
    // console.log("getUniPool error:", err);
...
async function getToken(app, tokenAddress, stakingAddress) {
  ...
  const type = await redis.get(tokenAddress);
  //getTokenWeights
  ...
  try {
    ...
    await redis.set(tokenAddress, "gelato");
    return gelatoPool;
  }
  catch(err) {
    //console.log(`couldn't match ${tokenAddress} ${i++}`, JSON.stringify(err, Object.getOwnPropertyNames(err)));
  }
  try {
    const pool = new ethcall.Contract(tokenAddress, UNI_ABI);
    ...
    await redis.set(tokenAddress, "uniswap");
    return uniPool;
  }
  catch(err) {
    // console.log(`couldn't match ${tokenAddress} ${i++}`, JSON.stringify(err, Object.getOwnPropertyNames(err)));
  ...
  try {
    ...
    await redis.set(tokenAddress, "balancer");
    return balPool;
  }
  catch(err) {
    //console.log(`couldn't match ${tokenAddress} ${i++}`, JSON.stringify(err, Object.getOwnPropertyNames(err)));
  ...
  try {
    ...
    await redis.set(tokenAddress, "balancerSmart");
    return balPool;
  }
  catch(err) {
    //console.log(`couldn't match ${tokenAddress} ${i++}`, JSON.stringify(err, Object.getOwnPropertyNames(err)));
  ...
  try {
    ...
    await redis.set(tokenAddress, "jar");
    return res;
  }
  catch(err) {
    //console.log(`couldn't match ${tokenAddress} ${i++}`, JSON.stringify(err, Object.getOwnPropertyNames(err)));
  ...
  try {
    ...
    await redis.set(tokenAddress, "cToken");
    return res;
  }
  catch(err) {
    //console.log(`couldn't match ${tokenAddress} ${i++}`, JSON.stringify(err, Object.getOwnPropertyNames(err)));
  ...
  try {
    ...
    await redis.set(tokenAddress, "vault");
    return res;
  }
  catch(err) {
    //console.log(`couldn't match ${tokenAddress} ${i++}`, JSON.stringify(err, Object.getOwnPropertyNames(err)));
  ...
  try {
    ...
    await redis.set(tokenAddress, "saddle");
    return res;
  }
  catch(err) {
    //console.log(`couldn't match ${tokenAddress} ${i++}`, JSON.stringify(err, Object.getOwnPropertyNames(err)));
  ...
  try {
    ...
    if (tokenAddress.toLowerCase() == "0x88E11412BB21d137C217fd8b73982Dc0ED3665d7".toLowerCase()) {
      ...
      await redis.set(tokenAddress, "curve");
      return res;
    }
    const [minter] = await app.ethcallProvider.all([crv.minter()]);
    const res = await getCurveToken(app, crv, tokenAddress, stakingAddress, minter);
  ...
  try {
    ...
    await redis.set(tokenAddress, "stableswap");
    return await getStableswapToken(app, stable, tokenAddress, stakingAddress);
  }
  catch (err) {
    //console.log(`couldn't match ${tokenAddress} ${i++}`, JSON.stringify(err, Object.getOwnPropertyNames(err)));
  ...
  try {
    ...
    await redis.set(tokenAddress, "erc20");
    return erc20tok;
  }
  catch(err) {
    //console.log(`couldn't match ${tokenAddress} ${i++}`, JSON.stringify(err, Object.getOwnPropertyNames(err)));
  ...
  try {
    ...
    await redis.set(tokenAddress, "dsToken");
    return res;
  }
  catch(err) {
...
function getGelatoPrices(tokens, prices, pool, chain="eth")
{
  ...
  var p0 = getParameterCaseInsensitive(prices,pool.token0) ? getParameterCaseInsensitive(prices,pool.token0).usd : null;
  var t1 = getParameterCaseInsensitive(tokens,pool.token1);
  var p1 = getParameterCaseInsensitive(prices,pool.token1) ? getParameterCaseInsensitive(prices,pool.token1).usd : null;
  if (p0 == null || p1 == null) {
    console.log(`Missing prices for tokens ${pool.token0} and ${pool.token1}.`);
    return undefined;
  }
  if ((t0 ? t0.decimals : null) == null) {
    console.log(`Missing information for token ${pool.token0}.`);
    return undefined;
  }
  if ((t1 ? t1.decimals : null) == null) {
...
{
  // console.log("pool.tokens:"/*, pool*/, pool.token0, pool.token1, "PRICES:", prices);
  var t0 = getParameterCaseInsensitive(tokens,pool.token0);
  var p0 = getParameterCaseInsensitive(prices,pool.token0) ? getParameterCaseInsensitive(prices,pool.token0).usd : null;
  var t1 = getParameterCaseInsensitive(tokens,pool.token1);
  var p1 = getParameterCaseInsensitive(prices,pool.token1) ? getParameterCaseInsensitive(prices,pool.token1).usd : null;
  ...
  if ((t0 && t0.decimals ? t0.decimals : null) == null) {
    console.log(`Missing information for token 0 ${pool.token0}.`);
    return undefined;
  }
  if ((t1 && t1.decimals ? t1.decimals : null) == null) {
    console.log(`Missing information for token 1 ${pool.token1}.`);
    return undefined;
  }
  var q0 = pool.q0 / 10 ** t0.decimals;
  var q1 = pool.q1 / 10 ** t1.decimals;
  ...
  return {
      ...
      print_price(chain="eth", decimals, customURLs) {
        ...
        if (customURLs) {
          ...
          _print(`${stakeTokenTicker} ${helperHrefs} Price: $${formatMoney(price)} TVL: $${formatMoney(tvl)}`);
          _print(`${t0.symbol} Price: $${displayPrice(p0)}`);
          _print(`${t1.symbol} Price: $${displayPrice(p1)}`);
...
function getBalancerPrices(tokens, prices, pool)
{
  ...
  var poolPrices = pool.poolTokens.map(t => getParameterCaseInsensitive(prices, t.address) ? getParameterCaseInsensitive(prices, t.address).usd : null);
...
function getWrapPrices(tokens, prices, pool)
{
  ...
  if (wrappedToken.token0 != null) { //Uniswap
    ...
    if (wrappedToken.token != null) { //e.g. stakedao crv token vault
      ...
      tokenPrice = getParameterCaseInsensitive(prices, wrappedToken.address) ? getParameterCaseInsensitive(prices, wrappedToken.address).usd : null;
    }
...
function getErc20Prices(prices, pool, chain="eth") {
  var price = getParameterCaseInsensitive(prices,pool.address) ? getParameterCaseInsensitive(prices,pool.address).usd : null;
  if (price === null) {
    // TODO: Log unique
    return null;
  }
  ...
  const getDexguruTokenlink =  function(network) {
    // const network = networks.location.pathname.split("/")[1]
  ...
  return {
    ...
    stakeTokenTicker : pool.symbol,
    print_price() {
      _print(`${name} Price: $${displayPrice(price)} Market Cap: $${formatMoney(tvl)} ${getDexguruTokenlink(chain)}`);
      _print(`Staked: ${pool.staked.toFixed(4)} ${pool.symbol} ($${formatMoney(staked_tvl)})`);
    },
    pair_links() {
      return {
...
function getCurvePrices(prices, pool, chain = null) {
  var price = (getParameterCaseInsensitive(prices,pool.token.address) ? getParameterCaseInsensitive(prices,pool.token.address).usd : 1) * pool.virtualPrice;
  if ((getParameterCaseInsensitive(prices, pool.address) ? getParameterCaseInsensitive(prices, pool.address).usd : 0) == 0) {
    prices[pool.address] = { usd : price };
  }
  var tvl = pool.totalSupply * price / 10 ** pool.decimals;
  var staked_tvl = pool.staked * price;
  ...
  return {
    ...
    stakeTokenTicker : pool.symbol,
    print_price() {
      _print(`${name} Price: $${formatMoney(price)} Market Cap: $${formatMoney(tvl)} ${getDexguruTokenlink(chain)}`);
      _print(`Staked: ${pool.staked.toFixed(4)} ${pool.symbol} ($${formatMoney(staked_tvl)})`);
    },
    print_contained_price() {
    }
...
async function getPoolInfo(app, chefContract, chefAddress, poolIndex, pendingRewardsFunction, showAll=false) {
  ...
  const poolToken = await getToken(app, poolInfo.lpToken ? poolInfo.lpToken : poolInfo.stakingToken, chefAddress);
  ...
  return {
      address: poolInfo.lpToken ? poolInfo.lpToken : poolInfo.stakingToken,
      allocPoints: poolInfo.allocPoint ? poolInfo.allocPoint : 1,
      poolToken: poolToken,
      userStaked : staked,
...
function printChefPool(App, chefAbi, chefAddr, prices, tokens, poolInfo, poolIndex, poolPrices,
  ...
  const staked_tvl = sp ? sp.staked_tvl : poolPrices.staked_tvl;
  ...
  if (poolInfo.userLPStaked > 0 && sp) {
    sp.print_contained_price(userStaked);
  }
...
async function loadChefContract(App, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  const chefContract = chef ? chef : new ethers.Contract(chefAddress, chefAbi, App.provider);
  ...
  const rewardsPerWeek = rewardsPerWeekFixed ? rewardsPerWeekFixed :
  ...
  var tokenAddresses = [].concat.apply([], poolInfos.filter(x => x ? x.poolToken : null).map(x => x.poolToken.tokens));
  ...
  const poolPrices = poolInfos.map(poolInfo => (poolInfo ? poolInfo.poolToken : null) ? getPoolPrices(tokens, prices, poolInfo.poolToken) : undefined);
...
async function loadBoardroom(App, prices, boardroomAddress, oracleAddress, lptAddress, rewardTokenAddress, stakeTicker, rewardTicker,
    ...
    const sharePrice = getParameterCaseInsensitive(prices, share) ? getParameterCaseInsensitive(prices, share).usd : null;
  ...
  rewardTokenFunction, stakeTokenFunction) {
    try {
      ...
      for (const key in newPrices) {
        if (newPrices[key] ? newPrices[key].usd : null)
      ...
      const stakeTokenPrice =
          prices[stakeTokenAddress] ? prices[stakeTokenAddress].usd :
            (getParameterCaseInsensitive(prices, stakeTokenAddress) ? getParameterCaseInsensitive(prices, stakeTokenAddress).usd : null);
      const rewardTokenPrice = getParameterCaseInsensitive(prices, rewardTokenAddress) ? getParameterCaseInsensitive(prices, rewardTokenAddress).usd : null;
...
async function loadMultipleSynthetixPools(App, tokens, prices, pools) {
  ...
  return { staked_tvl : totalStaked, totalUserStaked, totalAPR, infos };
}
