...
consoleInit(main)
  ...
  async function main() {
    ...
    /*let cp = await loadChefContract(App, null, CVX_CHEF_ADDR, CVX_CHEF_ABI,
      ...
      console.log(`Total staked: $${formatMoney(csp.staked_tvl)}`);
    ...
    console.log(`Total staked: $${formatMoney(p.staked_tvl+p0.staked_tvl+p1.staked_tvl)}`);
...
async function printConvexSynthetixPool(App, tokens, prices, info, chain="eth", customURLs) {
    ...
    if (stakeTokenTicker != "ETH") {
      console.log(`Stake ${info.userUnstaked.toFixed(6)} ${stakeTokenTicker}`, approveTENDAndStake)
    }
    else {
      _print("Please use the official website to stake ETH.");
    }
    console.log(`Unstake ${info.userStaked.toFixed(6)} ${stakeTokenTicker}`, unstake)
    console.log(`Claim ${info.earned.toFixed(6)} ${info.rewardTokenTicker} ($${formatMoney(info.earned*info.rewardTokenPrice)})`, claim)
    if (stakeTokenTicker != "ETH") {
...
async function printCrvSushiPool(App, info, chain="eth", customURLs) {
    ...
    _print(`<a target="_blank" href="https://etherscan.io/address/${info.stakingAddress}#code">Etherscan</a>`);
    console.log(`Unstake ${info.userStaked.toFixed(6)} ${info.stakeTokenTicker}`, unstake)
    console.log(`Claim ${info.earnings[0].toFixed(2)} ${info.rewardTokenTickers[0]} ($${formatMoney(info.earnings[0]*info.rewardTokenPrices[0])}) + ${info.earnings[1].toFixed(2)} ${info.rewardTokenTickers[1]} ($${formatMoney(info.earnings[1]*info.rewardTokenPrices[1])})`, claim)
