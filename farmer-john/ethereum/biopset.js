  ...
  async function printUnicryptPool(App, info, poolPrices, stakeTokenTicker, staked_tvl, stakeTokenPrice, rewardTokenPrice,
    usdPerWeek) {
      ...
      if (stakeTokenTicker != "ETH") {
        console.log(`Stake ${info.userUnstaked.toFixed(8)} ${stakeTokenTicker}`, approveTENDAndStake)
      }
      else {
        _print("Please use the official website to stake ETH.");
      }
      console.log(`Unstake ${info.userStaked.toFixed(8)} ${stakeTokenTicker}`, unstake)
      console.log(`Claim ${info.earned.toFixed(8)} ${info.rewardTokenTicker} ($${formatMoney(info.earned*rewardTokenPrice)})`, claim)
      if (stakeTokenTicker != "ETH") {
