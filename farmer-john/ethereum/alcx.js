...
const { init_ethers, getNewPricesAndTokens, getParameterCaseInsensitive, formatMoney, storeData, getToken, lookUpTokenPrices, getPoolPrices, printAPR } = require('./ethers_helper');
...
function printAlcxContractLinks(App, alcxAbi, alcxAddr, poolIndex, poolAddress,
  ...
  console.log(`Deposit ${unstaked.toFixed(fixedDecimals)} ${stakeTokenTicker}`)
  console.log(`Withdraw ${userStaked.toFixed(fixedDecimals)} ${stakeTokenTicker}`)
  console.log(`Claim ${pendingRewardTokens.toFixed(fixedDecimals)} ${rewardTokenTicker} ($${formatMoney(pendingRewardTokens*rewardTokenPrice)})`)
...
async function loadAlcxSushiContract(App, chef, chefAddress, chefAbi, rewardTokenFunction, rewardsPerWeekFixed, pendingRewardsFunction,
  ...
  console.log(`<a href='https://etherscan.io/address/${chefAddress}' target='_blank'>Staking Contract</a>`);
...
function printAlcxSushiAPR(rewardPrices, alcxRewardsPerWeek, sushiRewardsPerWeek,
  ...
  console.log(`${rewardTokenTickers[0]} Per Week: ${alcxRewardsPerWeek.toFixed(2)} ($${formatMoney(usdAlcxPerWeek)}) APR: Year ${yearlyAlcxAPR.toFixed(2)}%`);
  console.log(`${rewardTokenTickers[1]} Per Week: ${sushiRewardsPerWeek.toFixed(2)} ($${formatMoney(usdSushiPerWeek)}) APR: Year ${yearlySushiAPR.toFixed(2)}%`);
  console.log(`Total Per Week: $${formatMoney(totalUsdPerWeek)}`);
  console.log(`Total APR: Day ${totalDailyAPR.toFixed(2)}% Week ${totalWeeklyAPR.toFixed(2)}% Year ${totalYearlyAPR.toFixed(2)}%`);
  var userStakedUsd = userStaked * poolTokenPrice;
  var userStakedPct = userStakedUsd / staked_tvl * 100;
  console.log(`You are staking ${userStaked.toFixed(fixedDecimals)} ${stakeTokenTicker} ($${formatMoney(userStakedUsd)}), ${userStakedPct.toFixed(2)}% of the pool.`);
  ...
  if (userStaked > 0) {
    console.log(`Total Earnings: Day ${totalusdCoinsPerDay.toFixed(4)}% Week ${totalusdCoinsPerWeek.toFixed(2)}% Year ${totalusdCoinsPerYear.toFixed(2)}%`);
  }
  return {
    userStakedUsd,
...
function printAlcxSushiContractLinks(App, chefAbi, chefAddr, poolIndex, poolAddress,
  ...
  console.log(`Claim ${pendingRewardTokens[0].toFixed(fixedDecimals)} ${rewardTokenTickers[0]} ($${formatMoney(pendingRewardTokens[0]*rewardTokenPrices[0])}) + ${pendingRewardTokens[1].toFixed(fixedDecimals)} ${rewardTokenTickers[1]} ($${formatMoney(pendingRewardTokens[1]*rewardTokenPrices[1])})`, claim)
  console.log(`Staking or unstaking also claims rewards.`)
  console.log("");
}
