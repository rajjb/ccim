...
function printRulerContractLinks(App, rulerAbi, rulerAddr, lpAddress, lpAddress,
  ...
  _print(etherscanUrl);
  console.log(`Deposit ${unstaked.toFixed(fixedDecimals)} ${stakeTokenTicker}`, approveAndDeposit)
  console.log(`Withdraw ${userStaked.toFixed(fixedDecimals)} ${stakeTokenTicker}`, withdraw)
  let claimText = "";
  ...
  console.log(`Claim ${claimText}`, claim);
...
async function loadXRuler(App, tokens, prices) {
  ...
  console.log(`Deposit ${ruler.unstaked.toFixed(2)} RULER`, approveAndDeposit)
  console.log(`Withdraw ${(userBalance / 1e18).toFixed(2)} xRULER`, withdraw)
  _print(`\n`);
}
