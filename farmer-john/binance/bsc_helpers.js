...
const { init_ethers, lookUpPrices, getPoolPrices, printChefPool, formatMoney } = require('../ethereum/ethers_helper');
const { CURVE_ABI, BUNICORN_STABLE_POOL_ABI, ERC20_ABI, BALANCER_POOL_ABI, UNI_ABI } = require('../abis');
...
async function getBscBunicornStablePool(App, pool, poolAddress, stakingAddress, tokens) {
    ...
    const token0Balance = ethers.BigNumber.from(tokenBalances[0]).toString();
    const token1Balance = ethers.BigNumber.from(tokenBalances[1]).toString();
    const totalTokenBalance = parseFloat(token0Balance)  + parseFloat(token1Balance);
    for (let i = 0; i < tokens.length; i++) {
        const tokenWeightPercent = parseFloat(ethers.BigNumber.from(tokenBalances[i]).toString()) / totalTokenBalance;
...
async function getBscToken(App, tokenAddress, stakingAddress) {
  ...
  try {
    ...
    await redis.set(tokenAddress, "value");
    return valuePool;
  }
  catch(err) {
    // console.log(1, err);
  ...
  try {
    ...
    await redis.set(tokenAddress, "swap");
    return res;
  }
  catch(err) {
    // console.log(4, err);
  ...
  try {
    ...
    await redis.set(tokenAddress, "bscVault");
    return vault;
  }
  catch(err) {
    // console.log(5, err);
  ...
  try {
    ...
    await redis.set(tokenAddress, "bscCurve");
    return res;
  }
  catch(err) {
    // console.log(6, err);
  ...
  try {
      ...
      await redis.set(tokenAddress, "bunicorn:stable");
      return stablePool;
  }catch(err) {
    // console.log(7, err);
  }
  try {
    ...
    console.log(`Couldn't match ${tokenAddress} to any known token type.`, err);
  }
}
...
async function loadBscSynthetixPoolInfo(App, tokens, prices, stakingAbi, stakingAddress,
    rewardTokenFunction, stakeTokenFunction) {
      ...
      const stakeTokenPrice =
          prices[stakeTokenAddress] ? prices[stakeTokenAddress].usd : (getParameterCaseInsensitive(prices, stakeTokenAddress) ?
          getParameterCaseInsensitive(prices, stakeTokenAddress).usd : null);
...
async function getBscPoolInfo(App, chefContract, chefAddress, poolIndex, pendingRewardsFunction) {
  try {
    ...
    if (poolInfo.allocPoint == 0 || poolIndex == 105) {
      return {
        address: poolInfo.lpToken ? poolInfo.lpToken : poolInfo.token,
    ...
    const poolToken = await getBscToken(App, (poolInfo.lpToken ? poolInfo.lpToken : poolInfo.token), chefAddress);
...
async function loadBscChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  console.log(`<a href='https://bscscan.com/address/${chefAddress}' target='_blank'>Staking Contract</a>`);
  ...
  const poolInfos = (await Promise.all([...Array(poolCount).keys()].map(async (x) =>
    await getBscPoolInfo(App, chefContract, chefAddress, x, pendingRewardsFunction)))).filter(x => x != null);
  ...
  // }
  return { prices, totalUserStaked, totalStaked, averageApr, infos: aprs }
...
const bscTokens = [
  ...
  { "id": "pantherswap", "symbol": "PANTHER", "contract": "0x1f546ad641b56b86fd9dceac473d1c7a357276b7" },
]
