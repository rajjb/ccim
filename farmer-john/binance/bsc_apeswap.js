...
const { init_ethers } = require('../ethereum/ethers_helper');
const { getBscPrices, loadBscChefContract, getBscToken, getPoolPrices, printChefPool } = require('./bsc_helpers');
...
module.exports = async function main() {
  const App = await init_ethers(networks.NETWORKS.BINANCE_SMART_CHAIN.rpcUrls[0]);
 ...
 const rewardsSoulPerWeek = await APESWAP_SOUL_CHEF.rewardPerBlock() /1e18
  ...
  const resultBsc = await loadBscChefContract(App, tokens, prices, APESWAP_CHEF, APESWAP_CHEF_ADDR, APESWAP_CHEF_ABI, rewardTokenTicker,
      "cake", null, rewardsPerWeek, "pendingCake", [1]);

  const resultSoul = await loadSoulChefContract(App, tokens, prices, APESWAP_SOUL_CHEF, APESWAP_SOUL_CHEF_ADDR, APESWAP_SOUL_CHEF_ABI, rewardSoulTokenTicker,
      "rewardToken", null, rewardsSoulPerWeek, "pendingReward");

  const result = resultBsc.infos.concat(resultSoul.infos);

  for (const info of result) {
    info.name = info.stakeTokenTicker;
    info.rewardName = info.rewardTokenTicker;
    info.price = info.rewardTokenPrice;
    info.tvl = info.totalStakedUsd;
...
async function loadSoulChefContract(App, tokens, prices, chef, chefAddress, chefAbi, rewardTokenTicker,
  ...
  return { prices, totalUserStaked, totalStaked, averageApr, infos }
